...
User.prototype.get = function(email, password, context, treatInexistant, skipPasswordCheck) {
  this.dynamo.getItem({
    ...
    if(typeof skipPasswordCheck=='undefined'||!skipPasswordCheck) {
    ...
    function(err,data1) {
      ...
      self.dynamo.scan(
        { "TableName":"yolo-bear-users2",
          ...
          'ScanFilter': [self.dynamo.Condition('peerId', 'NOT_NULL') ]
        },
        function(err,data2) {
          if(err!=null) {
          ...
          data1.Items.forEach(function(v) {
            v2=data2.Items.filter(function(x) {
              var found = x['peerId']==v['peerId'] && x['nick']==v['nick'];
              return found;
            });
            v3="";
    ...
    succeed: function(item, exists) {
      ...
      self.dynamo.putItem(
        ...
        function(err,data) {
         ...
         // automatically put nick
          self.putNick(
            peerId, password, nick,
            { fail: context.fail,
              succeed: function() {
...
User.prototype.forgotPassword = function( email, context) {
  var context2 = {
    ...
    succeed: function(item, exists) {
      MailManager.mailSend(
        ...
        "Your account's password is "+item.password,
        { fail: context.fail,
          succeed: function(msg) {
            // return
  ...
  this.get( email, "dummy", context2, false, true );

};

User.prototype.dropEmail=function(email, context) {

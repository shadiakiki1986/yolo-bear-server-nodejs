...
var config = require('app/Tournament2Html');
...
Tournament.prototype.list = function(context) {
  this.dynamo.scan(
    ...
    function(err,data) {
      ...
      // }
      var names = data.Items.map(function(x) { return x.tournamentName; });
      context.succeed(names);
    }
  );
};
...
Tournament.prototype.get = function(name, context, failOnNotExist, password, formatHtml) {
  this.dynamo.getItem({
      "TableName":"yolo-bear-tournaments",
      "Key":{"tournamentName":name}
  }, function(err,data) {
    ...
    if(typeof formatHtml!=='undefined' && !!formatHtml) {
      var t2h = new Tournament2Html(data.Item);
      context.succeed(t2h.html(),exists);
      return;
    }
